...
1. **Step 3's boundary is wider than the paper's.** Eq. 6 normalizes over **one training batch**; this implementation normalizes over **the whole rollout**. The reason is upstream: `actor.py` collects `num_rollout_minis` windows of `global_batch_size / dp_size`, merges them with `concat_rollout_batches`, and only then calls `compute_advantages_and_returns` (its own comment: *we may need normalize the whole rollout*); `data.py` splits the merged result back into optimizer steps afterwards. The cross-DP all-reduce of `count/sum/sumsq` does make the statistics cover **every** rank — but what they cover is the rollout, not one training batch.

   The two coincide only when `rollout_batch_size * n_samples_per_prompt == global_batch_size`. `examples/gdpo/run-qwen3-0.6B-1xgpu-gdpo.sh` uses `4 * 8` against `16`, so `num_rollout_minis = 2` and one whitening spans two optimizer steps. Matching the paper exactly would mean teaching the whitening about mini boundaries (`ROLLOUT_MINI_LOCAL_SAMPLE_COUNTS_KEY` already carries them), which touches the advantage path every algorithm shares, and is follow-up work. **`--fully-async` is rejected during argument validation**: it hands advantage computation to the single-replica Advantages deployment, which has no data-parallel group and consumes one `global_batch_size / num_iters_per_train_update` slice at a time — when that quotient is 1 the whitened output is identically zero and the run trains on no signal at all, quietly. Supporting it needs cohort accumulation at the training-batch boundary, which is follow-up work.
